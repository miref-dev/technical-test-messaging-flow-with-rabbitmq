...
export default function MessageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchMessage = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Auto-mark as read if first time opening a delivered message
    ...
    // Re-check periodically for status updates if not fixed
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className={`w-2 h-2 rounded-full ${
                  message.status === 'read' ? 'bg-cyan-400' :
                  ...
                  {timeline.map((step, i) => (
                    <div key={step.label} className="flex flex-col items-center gap-3 relative z-10 w-24">
                      <div className={`w-4 h-4 rounded-full border-2 ${
                        step.active ? 'bg-indigo-500 border-indigo-400' : 'bg-[#0f172a] border-slate-700'
                      }`} />
